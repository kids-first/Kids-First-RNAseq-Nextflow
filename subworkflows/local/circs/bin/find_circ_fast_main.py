...
def find_breakpoints(A,B,read,genome,chrom,asize,margin,maxdist):
    ...
    # e.g., check positions at 0 and middle if length > 4
    anchor_idxs = (0,)
    if l > 8:
        anchor_idxs = (0, l // 2)
    ...
    for x in xrange_local(l + 1):
        ...
        # do direct memoryview indexing (no allocations)
        anchor_miss = False
        for ai in anchor_idxs:
            if spliced_mv_local[ai] != internal_mv_local[ai]:
                ...
                break
        if anchor_miss:
            # If expected mismatch counts are likely large, early skip
            # (but still continue if anchor matches)
            # This avoids calling the full mismatch routine for obvious misses
            continue
...
def output(cand,prefix, suffix, wiggle, min_uniq_qual):
    ...
    for c,hit in cand.items():
        ...
        n_reads,n_uniq,best_qual,best_other,spliced_at_begin,spliced_at_end,tissues,tiss_counts,min_edit,min_anchor_ov,n_hits = hit.scores(chrom,start,end,sense, wiggle)
        
        if best_other < min_uniq_qual:
            N['anchor_not_uniq'] += 1
            continue
...
def main():
    ...
    try:
        for A,B in grouper(2,sam):
            ...
            if (A.is_reverse and dist > 0) or (not A.is_reverse and dist < 0):
                ...
                bp = find_breakpoints(A,B,read,genome,chrom,options.asize,options.margin,options.maxdist)
                if not bp:
                    N['circ_no_bp'] += 1
                else:
                    N['circ_reads'] += 1
                ...
                for h in bp:
                    ...
                    circs[h].add(read,A,B,dist,ov,n_hits,minmapscore,samples)
            ...
            if (A.is_reverse and dist < 0) or (not A.is_reverse and dist > 0):
                ...
                for h in bp:
                    ...
                    splices[h].add(read,A,B,dist,ov,n_hits,minmapscore,samples)
    ...
    output(circs,"circ",options.wiggle,options.min_uniq_qual)
    output(splices,"norm",options.wiggle,options.min_uniq_qual)
