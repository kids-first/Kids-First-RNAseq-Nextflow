...
only_num_heat_dcc=heat_dcc[ , tokeep_dc, drop = FALSE]



clean_only_q=only_num_heat_dcc[,(!(colnames(only_num_heat_dcc) %in% unused_colnames)), drop = FALSE ]# filtering out bad colnames- besides the non-numeric ones
# read the reads file
...
normalize_only_quants_df <- function(quants_df,n_readsmil_ordered){
  ...
  new_df=new_df[,(colnames(new_df)!="ncol"), drop = FALSE]
